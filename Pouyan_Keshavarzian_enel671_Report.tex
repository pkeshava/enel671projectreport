...
where
$$\sigma^2_d$$ is the deterministic variance and the tap weight vector is
...
Which in the context of this project is the randomly generated BPSK signal.
\subsection{Effect of Eigenvalue Spread}
The LMS algorithm was run with total K = 500 iterations and the error for each value of the tap input vector n = 1,2,...N was averaged. The error was then used to calculate the mean
square error using the formula:
$$MSE=\dfrac{1}{K}\sum_{k=1}^{K}e_k^2(n); \qquad n = 1,2,....N$$
This is known as the learning curve of the algorithm. The learning curve is used to judge the
...
\end{figure}
These results show that the LMS is very suceptible to eigenvalue spread
